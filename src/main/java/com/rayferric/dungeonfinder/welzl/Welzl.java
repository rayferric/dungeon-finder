...
public class Welzl {
    ...
    public static Sphere run(List<Vector3f> points) {
        ...
        return search(points, new ArrayList<>(), points.size());
    }
     ...
     */
    private static Sphere search(List<Vector3f> points, List<Vector3f> support, int pointCount) {
        // If we've ran out of points or filled the support set, we can finally use the trivial method:
        if (pointCount == 0 || support.size() == 4)
            return trivial(support.toArray(new Vector3f[0]));

        // Pop a point and compute a bounding sphere without it.
        Vector3f point = points.get(pointCount - 1);
        Sphere sphere = search(points, new ArrayList<>(support), pointCount - 1);

        // If the popped point is inside, we can simply drop it:
        if (sphere.contains(point)) return sphere;

        // Otherwise, the popped point must lie on the boundary,
        // therefore we add it to the support set:
        support.add(point);
        return search(points, support, pointCount - 1);
    }
